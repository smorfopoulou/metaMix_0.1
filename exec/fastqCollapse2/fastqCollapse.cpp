...
parameters::parameters (const int narg, char ** argc) {
 ...
 ///-------------
  ...
  for (int i = 0; i != temp.size(); i++) {
    ...
    if ( temp[i][0] == "-sigLength" ) {
      flag = true;
      if (temp[i].size() != 2) {cerr<<"Needs 1 argument for -sigLength\n";exit(1);}
      sig_length = atoi(temp[i][1].c_str());
    }
...
int main (int narg, char ** argc) {
  ...
  cout<<"Length of the tag used to define the signature of a read: "<<my_params->sig_length<<endl;
  ...
  for (int parse = 0; parse != 2; parse++) {
	...
	if (gzeof(my_pointer2)) break;
      ...
      if (parse == 0) {
	int sig1 = min((int) seq1.size(), my_params->sig_length);
	int sig2 = min((int) seq2.size(), my_params->sig_length);
