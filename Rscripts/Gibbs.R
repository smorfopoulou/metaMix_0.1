...
#iter: number of  gibbs cycles
#species: taxon ids of species that are assumed present in the sample
#abund: Intial values for the mixing weights
#hyperParam: parameters for prior Dirichlet
############################################################################################################################################################

gibbsSparseFinal = function(pij, iter, species, abund, pijUnknown, hyperParam)
{
  time1 <- Sys.time() 
...
######## STEP2. Sample from multinomial ~ Mult(draw=1,objects = 1, prob = zij)
    ...
    assignm<-fast.rmultinom.weight(proba.matrix=zij, z.matrix=assignm, seed=1, weights=as.vector(w))
